...
import ExpertHasilTestTable from '@/app/(dashboard)/components/ExpertHasilTestTable'
...
export default function HasilTes() {
  ...
  const filteredData = React.useMemo(() => {
    ...
    if (filters.startDate) {
      result = result.filter((item) => {
        const date = new Date(item?.started_at ?? 0)
    ...
    if (filters.endDate) {
      result = result.filter((item) => {
        const date = new Date(item?.completed_at ?? item?.started_at ?? 0)
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      <div className="w-full max-w-7xl mx-auto space-y-6 p-6">
        ...
        <ExpertHasilTestTable tableData={filteredData} />
      </div>
    </div>
  )
}
